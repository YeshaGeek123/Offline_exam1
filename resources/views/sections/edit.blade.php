    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-8 col-xlg-9 col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form class="form-horizontal form-material" method="POST" action="{{ route('sections.update', $section->id) }}">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="col-md-12 border-bottom p-0">
                                    <input type="text" class="form-control p-0 border-0" name="title" placeholder="Enter name" value="{{ $section->title }}" required> 
                                </div>
                            </div>
                            <!-- <div class="form-group mb-4">
                                ...
                                <div class="col-sm-12 border-bottom">
                                    <select class="form-select shadow-none p-0 border-0 form-control-line" name="role_id">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}" {{ $section->role_id == $role->id ? 'selected' : '' }}>{{ $role->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div> -->
